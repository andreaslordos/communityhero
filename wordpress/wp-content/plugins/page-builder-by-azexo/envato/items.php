 ...
 */
if (!class_exists('AZEXO_Envato_Market_Items')) :

    /**
     * Creates the theme & plugin arrays & injects API results.
     *
     * @class AZEXO_Envato_Market_Items
     * @version 1.0.0
     * @since 1.0.0
     */
    class AZEXO_Envato_Market_Items {
        ...
        /**
         * The AZEXO_Envato_Market_Items Instance
         *
         * Ensures only one instance of this class exists in memory at any one time.
         *
         * @see AZEXO_Envato_Market_Items()
         * @uses AZEXO_Envato_Market_Items::init_actions() Setup hooks and actions.
         *
         * @since 1.0.0
         * @static
         * @return object The one true AZEXO_Envato_Market_Items.
         * @codeCoverageIgnore
         */
         ...
         *
         * @see AZEXO_Envato_Market_Items::instance()
         *
         * @since 1.0.0
         * @access private
         * @codeCoverageIgnore
                ...
                foreach ($installed as $slug => $premium) {
                    ...
                    if ($theme->exists() && version_compare($theme->get('Version'), $premium['version'], '<')) {
                        $transient->response[$slug] = array(
                            ...
                            'url' => $premium['url'],
                            'package' => AZEXO_Envato_Market_API::instance()->deferred_download($premium['id']),
                        );
                    }
                }
            }
            ...
            if ('plugin_information' === $action && isset($args->slug)) {
                ...
                foreach ($installed as $slug => $plugin) {
                    if (dirname($slug) === $args->slug) {
                        ...
                        $response->download_link = AZEXO_Envato_Market_API::instance()->deferred_download($plugin['id']);
                        break;
                    }
                }
            }
        ...
        public function set_themes($forced = false, $use_cache = false) {
            self::$themes = get_site_transient(azexo_updater_option() . '_themes');

            if (false === self::$themes || true === $forced) {
                $themes = AZEXO_Envato_Market_API::instance()->themes();
                foreach (azexo_envato_market_items() as $item) {
                    if (empty($item)) {
                        continue;
                    }
                    if ('theme' === $item['type']) {
                        ...
                        $request = AZEXO_Envato_Market_API::instance()->item($item['id'], $request_args);
                        if (false !== $request) {
                            $themes[] = $request;
                        }
                    }
        ...
        public function set_plugins($forced = false, $use_cache = false, $args = array()) {
            self::$plugins = get_site_transient(azexo_updater_option() . '_plugins');

            if (false === self::$plugins || true === $forced) {
                $plugins = AZEXO_Envato_Market_API::instance()->plugins();
                foreach (azexo_envato_market_items() as $item) {
                    if (empty($item)) {
                        continue;
                    }
        ...
        private function process_themes($purchased) {
            ...
            set_site_transient(azexo_updater_option() . '_themes', self::$themes, HOUR_IN_SECONDS);
        }
        ...
        private function process_plugins($purchased, $args = array()) {
            ...
            set_site_transient(azexo_updater_option() . '_plugins', self::$plugins, HOUR_IN_SECONDS);
        }
