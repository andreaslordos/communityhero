...
function wc_maybe_reduce_stock_levels( $order_id ) {
	...
	$area = $order->get_shipping_state();
	$phone = $order->get_billing_phone();
	$full_address = $address . ' ' . $area . ' ' . $city;
	$result = array("from"=>$phone, "address"=>$full_address);
	...
	*/
	$phone = $order->get_billing_phone();
	$url = "https://rhubarb-cake-22341.herokuapp.com/api/v1/sms/order/";
	$totalStr = '';
	foreach ( $order->get_items() as $item_id => $item ) {
		$name = $item->get_name();
		$totalStr = $totalStr . $name . "\n";
	}

	$totalStr = substr($totalStr, 0, -2);

	error_log($totalStr);
	$result2 = array("from"=>$phone, "content"=>$totalStr);
	$payload2 = json_encode($result2);
	$ch = curl_init($url);
	curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type:application/json'));
	//curl_setopt($ch, CURLOPT_POSTFIELD, '{\"from\": \"$phone\", \"content\": \"$totalStr\"}');
