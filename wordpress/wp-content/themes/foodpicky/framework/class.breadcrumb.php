...
class AZEXO_Breadcrumb {
    ...
    public function get_breadcrumb() {
        return apply_filters('azexo_get_breadcrumb', $this->crumbs, $this);
    }
    ...
    private function add_crumbs_404() {
        $this->add_crumb(esc_html__('Error 404', 'foodpicky'));
    }
    ...
    private function add_crumbs_tag() {
        ...
        $this->add_crumb(sprintf(esc_html__('Posts tagged &ldquo;%s&rdquo;', 'foodpicky'), single_tag_title('', false)), get_tag_link($queried_object->term_id));
    }
    ...
    private function add_crumbs_author() {
        ...
        $this->add_crumb(sprintf(esc_html__('Author: %s', 'foodpicky'), $userdata->display_name));
    }
    ...
    private function search_trail() {
        if (is_search()) {
            $this->add_crumb(sprintf(esc_html__('Search results for &ldquo;%s&rdquo;', 'foodpicky'), get_search_query()), remove_query_arg('paged'));
        }
    }
    ...
    private function paged_trail() {
        if (get_query_var('paged')) {
            $this->add_crumb(sprintf(esc_html__('Page %d', 'foodpicky'), get_query_var('paged')));
        }
    }

}

function azexo_breadcrumb($args = array()) {
    $args = wp_parse_args($args, apply_filters('azexo_breadcrumb_defaults', array(
        'delimiter' => '&nbsp;&#47;&nbsp;',
        'wrap_before' => '<nav class="breadcrumb">',
        'wrap_after' => '</nav>',
        'before' => '',
        'after' => '',
        'home' => esc_html__('Home', 'foodpicky')
    )));

    $breadcrumbs = new AZEXO_Breadcrumb();

    if (!empty($args['home'])) {
        $breadcrumbs->add_crumb($args['home'], apply_filters('azexo_breadcrumb_home_url', esc_url(home_url('/'))));
    }
    ...
    if (!empty($breadcrumb)) {
        ...
        foreach ($breadcrumb as $key => $crumb) {
            ...
            if (!empty($crumb[1]) && sizeof($breadcrumb) !== $key + 1) {
                ...
                print '<span>' . esc_html($crumb[0]) . '</span>';
            }
